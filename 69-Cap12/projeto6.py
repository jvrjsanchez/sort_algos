# Projeto 6 - IA em Ativos Financeiros Para Prever Lucro por Ação (EPS)

# Como solução para este Projeto usaremos um modelo de IA com arquitetura LSTM (Long Short-Term Memory)
...
eps = df_dsa['eps'].values.reshape(-1, 1)
...
def dsa_cria_dataset(data, look_back = 1):
...
look_back = 1
X_treino, y_treino = dsa_cria_dataset(dados_treino_norm, look_back)
X_teste, y_teste = dsa_cria_dataset(dados_teste_norm, look_back)

# Reshape dos dados para [samples, time steps, features]. Isso é um requerimento do modelo LSTM
X_treino = np.reshape(X_treino, (X_treino.shape[0], X_treino.shape[1], 1))
X_teste = np.reshape(X_teste, (X_teste.shape[0], X_teste.shape[1], 1))

# Veja a definição das LSTMs no videobook do Capítulo 12 do curso.

# As LSTMs processam sequências de dados uma etapa de tempo por vez. 
# Ao fornecer os dados no formato [samples, time steps, features], você está dizendo ao modelo quantas sequências ele deve processar, 
# quantos passos de tempo existem em cada sequência e quantas características existem em cada passo de tempo.
...
for _ in range(2):  
    ...
    # Isso significa que usaremos a previsão de 2024 para prever o valor de 2025
...
print("\nProjeto Concluído. Obrigado, DSA!")
